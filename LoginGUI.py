...
from RegisterGUI import Ui_RegisterWindow
from ExamGUI import Ui_ExamWindow
...
class Ui_LoginWindow(object):
    ...
    def SignIn(self): #This function check if the enter info is have account 
        ...
        if (LoginBL.GetUserExist() == False):
            if LoginBL.IsUsernameBlank(self.lineEdit_Username.text()): #check if no input
                self.Label_StatusSignIn.setStyleSheet("color: red;")
                self.Label_StatusSignIn.setText("Empty Username!")
                
            elif LoginBL.IsUsernameInData(self.lineEdit_Username.text()):
                LoginBL.SetUsername(self.lineEdit_Username.text())
                LoginBL.SetUserExist(True)
        ...
        elif (LoginBL.GetUserExist() == True):
            
            if LoginBL.IsPasswordBlank(self.lineEdit_Username.text()):
                self.Label_StatusSignIn.setStyleSheet("color: red;")
                self.Label_StatusSignIn.setText("Empty Password!")
                
            else:    
                if LoginBL.IsPasswordInData(self.lineEdit_Username.text()):
                    LoginBL.SetPassword(self.lineEdit_Username.text())
                    
                    self.Label_StatusSignIn.setStyleSheet("color: green;")
                    Message = "Password is correct!\nWelcome " + LoginBL.GetFirstName() + " " + LoginBL.GetLastName() + "!"
                    self.Label_StatusSignIn.setText(Message)
                    
                    QtTest.QTest.qWait(3000)
                    self.ExamWindow = QtWidgets.QMainWindow()
                    self.ei = Ui_ExamWindow()
                    self.ei.setupUi(self.ExamWindow,LoginBL.GetFirstName(), LoginBL.GetLastName())
    ...
    def GoToRegisterWin(self):
        ...
        self.abcd = Ui_RegisterWindow()
        self.abcd.setupUi(self.RegisterWindow)
        LoginWindow.hide()
